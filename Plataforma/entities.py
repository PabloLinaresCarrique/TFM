...
def show_entities_section(client_details):
    print("Received client details in show_entities_section:", client_details)  # Debug print
    st.write("## Entities")
    st.write(f"Debug: Received client_details: {client_details}")  # Debug print
    ...
    if "entity_tabs" not in st.session_state:
        ...
        st.session_state.entity_data = {
            "Main Client": {
                "name": client_details[5] if client_details else "Debug on entities.py",
                "age": client_details[1] if client_details else "Debug on entities.py",
                "job": client_details[2] if client_details else "Debug on entities.py",
                "marital_status": client_details[3] if client_details else "Debug on entities.py",
                "education": client_details[4] if client_details else "Debug on entities.py",
            }
        }
