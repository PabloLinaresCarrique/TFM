...
<br>
<br>
<p><b>Timestamp:</b> {alert_details[1] if len(alert_details) > 1 else 'N/A'}</p>
<p><b>From Bank:</b> {alert_details[2] if len(alert_details) > 2 else 'N/A'}</p>
<p><b>From Account:</b> {alert_details[3] if len(alert_details) > 3 else 'N/A'}</p>
<p><b>To Bank:</b> {alert_details[4] if len(alert_details) > 4 else 'N/A'}</p>
<p><b>To Account:</b> {alert_details[5] if len(alert_details) > 5 else 'N/A'}</p>
<p><b>Amount Received:</b> {alert_details[6] if len(alert_details) > 6 else 'N/A'} {alert_details[7] if len(alert_details) > 7 else ''}</p>
<p><b>Amount Paid:</b> {alert_details[8] if len(alert_details) > 8 else 'N/A'} {alert_details[9] if len(alert_details) > 9 else ''}</p>
<p><b>Payment Format:</b> {alert_details[10] if len(alert_details) > 10 else 'N/A'}</p>
<br>
"""
...
"""
    try:
        # Corrected OpenAI API call using client.chat.completions.create
        response = client.chat.completions.create(
            ...
            max_tokens=350,  # Increased to accommodate the additional suggested steps
    ...
    if st.button("Run Risk Analysis", key="generate_risk_factors"):
        with st.spinner("Generating risk factors..."):
            ...
            if risk_factors:
                ...
                st.session_state["narrative_content"] = updated_content  # Update session state
